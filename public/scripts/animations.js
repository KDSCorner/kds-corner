...
window.AnimationController = window.AnimationController || class AnimationController {
  constructor() {
    this.init();
  }
  ...
  init() {
    ...
    // Ensure hero typewriter gets a controller instance so it runs immediately
    try {
      const typewriterElements = document.querySelectorAll('.typewriter');
      if (typewriterElements.length > 0 && !window.__animationControllerInitialized) {
        window.__animationControllerInitialized = true;
      }
    } catch {}
  ...
  initHoverAnimations() {
    ...
    // Portfolio items subtle image parallax (avoid moving the grid item itself)
    const portfolioItems = document.querySelectorAll(".portfolio-item");
    portfolioItems.forEach((item) => {
      const image = item.querySelector(".portfolio-image img, .portfolio-image-placeholder");
      if (!image) return;
      ...
      item.addEventListener("mousemove", (e) => {
        // Skip effect if item is hidden by filtering
        if (item.classList.contains("hidden")) return;
        const rect = item.getBoundingClientRect();
        const x = e.clientX - rect.left - rect.width / 2;
        const y = e.clientY - rect.top - rect.height / 2;
        // very subtle movement
        targetX = Math.max(-10, Math.min(10, x * 0.05));
        targetY = Math.max(-10, Math.min(10, y * 0.05));
      });
...
// Auto-instantiate AnimationController so typewriter starts
(function initAnimationsWhenReady() {
  const init = () => {
    if (!window.__animationControllerInstance) {
      window.__animationControllerInstance = new window.AnimationController();
    }
  };
...
if (typeof module !== "undefined" && module.exports) {
  // In browser, use the global on window
  module.exports = { AnimationController: window.AnimationController, TextRevealAnimation, MorphingShapes };
}
